...
- [src/pages/userRealName/index.tsx:1-292](file://src/pages/userRealName/index.tsx#L1-L292)
- [src/pages/operatorRealName/index.tsx:1-282](file://src/pages/operatorRealName/index.tsx#L1-L282)
- [src/pages/ordinaryuserRealName/index.tsx:1-216](file://src/pages/ordinaryuserRealName/index.tsx#L1-L216)
- [src/pages/responsibleRealName/index.tsx:1-314](file://src/pages/responsibleRealName/index.tsx#L1-L314)
...
```mermaid
flowchart TD
Start(["进入经办人实名认证"]) --> PlatformSelect["选择平台<br/>CMP/DCP"]
...
DETAIL --> CARD["笔记卡片"]
PROFILE --> AVA["用户头像"]
USERREALNAME["用户实名认证"] --> SLIDER
OPERATORREALNAME["经办人实名认证"] --> PICKER["选择器组件"]
ORDINARYUSERREALNAME["普通用户实名认证"] --> SLIDER
RESPONSIBLEREALNAME["责任人实名认证"] --> PICKER
```
...
## 结论
本项目页面组件围绕"信息流 + 社交互动 + 实名认证"的核心场景构建，通过清晰的页面职责划分、统一的组件复用与完善的路由配置，实现了良好的可维护性与扩展性。新增的四个实名认证页面进一步完善了用户身份验证体系，支持不同用户类型的差异化需求。建议后续在真实接口接入时完善错误处理与缓存策略，并持续优化图片与列表渲染性能，以进一步提升用户体验。

## 附录
- 页面路由清单
  - 登录页、首页、发现页、发布页、消息页、个人中心、详情页、用户实名认证、经办人实名认证、普通用户实名认证、责任人实名认证
- 页面配置要点
