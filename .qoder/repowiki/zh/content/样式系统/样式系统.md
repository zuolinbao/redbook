...
- [src/styles/_reset.scss](file://src/styles/_reset.scss)
...
## 更新摘要
**所做更改**
- Sass依赖版本从1.75.0升级到1.77.8，解决JavaScript API弃用警告，确保构建稳定性与未来兼容性
- 新增完整的CSS重置系统（src/styles/_reset.scss），提供浏览器默认样式的统一重置
- 全局变量系统现代化重构，与Taroify主题保持完全一致的颜色体系
- 新增FormTitle组件样式系统，提供表单标题的统一视觉规范
- 完善Taroify框架样式覆盖系统，彻底移除蓝色焦点边框和轮廓
...
**更新** 新增CSS重置系统、现代化全局变量系统、FormTitle组件样式规范和Taroify框架样式覆盖系统的完善，以及Sass依赖版本升级至1.77.8

## 项目结构
样式相关文件主要分布于以下位置：
- 全局样式：src/app.scss
- CSS重置系统：src/styles/_reset.scss
...
```mermaid
graph TB
A["全局样式<br/>src/app.scss"] --> B["CSS重置系统<br/>src/styles/_reset.scss"]
A --> C["全局变量<br/>src/styles/_variables.scss"]
A --> D["页面样式<br/>src/pages/*/index.module.scss"]
A --> E["组件样式<br/>src/components/*/index.scss"]
A --> F["组件样式(CSS Modules)<br/>src/components/*/index.module.scss"]
A --> G["表单标题组件<br/>src/components/FormTitle/index.module.scss"]
H["构建脚本与依赖<br/>package.json"] --> I["TypeScript 路径别名<br/>tsconfig.json"]
J["样式检查配置<br/>stylelint.config.mjs"] --> A
K["视口与基础HTML<br/>src/index.html"] --> A
L["表单标题组件<br/>src/components/FormTitle/index.tsx"] --> G
M["用户实名认证页面<br/>src/pages/userRealName/index.tsx"] --> L
```

**图表来源**
- [src/app.scss:1-68](file://src/app.scss#L1-L68)
- [src/styles/_reset.scss:1-186](file://src/styles/_reset.scss#L1-L186)
- [src/styles/_variables.scss:1-53](file://src/styles/_variables.scss#L1-L53)
...
## 核心组件
- **新增** CSS重置系统
  - 提供完整的浏览器默认样式重置，确保跨平台一致性
  - 包含盒模型重置、HTML5元素display重置、列表样式重置等
  - 特别针对小程序平台的page元素进行专门优化
- 全局变量与命名空间
  - 使用 SCSS 的命名空间导入，集中管理主色、文本色、背景色、边框色与 TabBar 高度等变量，便于统一风格与主题切换
  - **现代化重构** 与Taroify主题保持完全一致的颜色体系，确保组件样式的一致性
- 全局样式
...
- [src/components/SliderVerify/index.module.scss:1-191](file://src/components/SliderVerify/index.module.scss#L1-L191)

## 架构总览
样式系统采用"CSS重置 + 全局变量 + 全局样式 + 页面模块 + 组件模块"的分层组织方式，并结合 CSS Modules 实现样式隔离。下图展示全局样式与页面/组件样式的依赖关系，包括新增的表单标题组件：

```mermaid
graph TB
RESET["CSS重置系统<br/>_reset.scss"] --> VARIABLES["全局变量<br/>_variables.scss"]
VARIABLES --> APP["全局样式<br/>app.scss"]
...
- [src/app.scss:1-68](file://src/app.scss#L1-L68)
- [src/components/FormTitle/index.module.scss:1-25](file://src/components/FormTitle/index.module.scss#L1-L25)
- [src/pages/userRealName/index.tsx:150](file://src/pages/userRealName/index.tsx#L150)
...
- **全新** 提供完整的浏览器默认样式重置
- 核心功能：
  - 盒模型重置：所有元素使用 border-box，统一盒模型计算
  - HTML元素重置：移除所有元素的 margin、padding、border，重置字体继承
  - HTML5元素display重置：确保语义化元素正确显示
  - 列表样式重置：统一ol、ul的列表样式
  - 引用样式重置：清除blockquote、q的引号样式
  - 表格样式重置：重置表格的边框合并和间距
  - 链接样式重置：移除默认链接装饰，继承颜色
  - 按钮样式重置：重置按钮的默认样式，使用inherit继承
  - 表单元素重置：统一input、textarea、select的字体继承
  - 图片重置：确保图片最大宽度100%，自动高度
  - 焦点轮廓重置：移除所有元素的焦点轮廓
  - 小程序特有重置：针对page元素的专门优化
  - 滚动条隐藏：隐藏Webkit滚动条但保留滚动功能

**章节来源**
- [src/styles/_reset.scss:1-186](file://src/styles/_reset.scss#L1-L186)

### 全局变量系统（现代化重构）
- **现代化重构** 与Taroify主题保持完全一致的颜色体系
- 颜色体系：
  - 基础色：$black、$white
  - 灰色系：$gray-1到$gray-8，提供完整的灰度渐变
  - 功能色：$red、$blue、$cyan、$green、$orange
  - 组件色：$primary-color、$info-color、$success-color、$warning-color、$danger-color
  - 文本色：$text-color、$text-color-2、$text-color-3
  - 背景色：$background-color、$background-color-2、$bg-color
  - 边框色：$border-color
  - 活跃与禁用：$active-color、$active-opacity、$disabled-opacity
  - TabBar高度：$tabbar-height
- 兼容性支持：保留旧变量名如$text-secondary、$text-light
- 与Taroify完全同步：确保第三方组件样式的一致性
...
- 样式特点：
  ...
  - 大字号标题文本，强调重要信息
  - 统一的间距和对齐规范
  - 使用全局变量系统中的$primary-color作为渐变色竖条颜色
...
- [src/styles/_variables.scss:51](file://src/styles/_variables.scss#L51)
- [src/components/CustomTabBar/index.module.scss:14](file://src/components/CustomTabBar/index.module.scss#L14)
...
- **新增** CSS重置系统优化：通过集中重置减少重复的样式声明，提升样式加载性能
- **新增** Sass版本升级优化：从1.75.0升级到1.77.8，解决JavaScript API弃用警告，提升构建稳定性和未来兼容性
...
- **新增** FormTitle 组件问题
  ...
  - 确认样式模块类名引用是否匹配
- **新增** CSS重置系统问题
  - 检查_reset.scss文件是否正确导入到app.scss中
  - 确认重置规则的优先级是否正确
- **新增** 全局变量系统问题
  - 检查_variables.scss中的颜色变量是否与Taroify组件一致
  - 确认变量命名是否符合新的命名规范
- **新增** Sass版本兼容性问题
  - 确认Sass版本1.77.8与构建工具链兼容
  - 检查是否存在JavaScript API弃用警告
...
- [pnpm-lock.yaml:154-155](file://pnpm-lock.yaml#L154-L155)

## 结论
红书项目的样式系统以 SCSS 为核心，结合CSS重置系统、全局变量与全局样式，采用页面与组件模块化组织，并通过 CSS Modules 实现强隔离。整体遵循移动端优先与多端适配原则，具备良好的可维护性与扩展性。

**更新** 最新改进包括全新的CSS重置系统、现代化的全局变量系统与Taroify主题完全一致、完善的 Taroify 框架样式覆盖系统，彻底解决了蓝色焦点边框问题，以及新增的 FormTitle 组件，提供了统一的表单标题样式规范。Sass依赖已升级至1.77.8版本，解决了JavaScript API弃用警告，确保构建过程的稳定性和未来兼容性。建议后续完善断点体系与主题变量覆盖能力，持续提升跨平台一致性与开发效率。
...
- **新增** 组件样式规范
  ...
  - 统一的组件样式命名和结构规范
- **新增** CSS重置系统使用规范
  - 所有新样式文件必须先导入_reset.scss
  - 遵循重置系统的重置原则，避免重复声明
- **新增** 全局变量系统使用规范
  - 严格使用_variables.scss中定义的颜色变量
  - 不得直接使用硬编码颜色值
  - 与Taroify组件保持颜色一致性
- **新增** Sass版本管理规范
  - 定期检查Sass版本兼容性，及时升级到最新稳定版本
  - 确保构建工具链与Sass版本匹配，避免API弃用警告
  - 在团队中统一Sass版本，避免本地环境差异导致的问题