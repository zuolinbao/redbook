...
F["构建脚本与依赖<br/>package.json"] --> G["TypeScript 路径别名<br/>tsconfig.json"]
H["样式检查配置<br/>stylelint.config.mjs"] --> A
I["视口与基础HTML<br/>src/index.html"] --> A
```
...
## 架构总览
样式系统采用“全局变量 + 全局样式 + 页面模块 + 组件模块”的分层组织方式，并结合 CSS Modules 实现样式隔离。下图展示全局样式与页面/组件样式的依赖关系：
...
示例页面与对应样式路径
- 首页：[src/pages/home/index.module.scss](file://src/pages/home/index.module.scss)
- 发现页：[src/pages/discover/index.module.scss](file://src/pages/discover/index.module.scss)
- 详情页：[src/pages/detail/index.module.scss](file://src/pages/detail/index.module.scss)
- 发布页：[src/pages/publish/index.module.scss](file://src/pages/publish/index.module.scss)
- 我的页：[src/pages/profile/index.module.scss](file://src/pages/profile/index.module.scss)
...
## 结论
红书项目的样式系统以 SCSS 为核心，结合全局变量与全局样式，采用页面与组件模块化组织，并通过 CSS Modules 实现强隔离。整体遵循移动端优先与多端适配原则，具备良好的可维护性与扩展性。建议后续完善断点体系与主题变量覆盖能力，持续提升跨平台一致性与开发效率。
